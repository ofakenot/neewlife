...
async function saveManualSheetLine(sellerId,rowNumber,card) {
    if(!canEditWgIk()) return showToast('Apenas WG e IK podem editar esta planilha.');
    const get = field => card.querySelector(`[data-sheet-field="${field}-${sellerId}-${rowNumber}"]`);
    const productName=(get('product')?.value||'').trim();
    const total=Number(get('total')?.value||0);
    const paid=Number(get('paid')?.value||0);
    const notes=(get('notes')?.value||'').trim();
    if(!productName && !total && !paid && !notes) return showToast('Linha vazia: preencha o produto e o valor total debitado.');
    if(!productName) return alert('Informe o nome do produto.');
    if(!(total>0) || paid<0 || paid>total) return alert('Informe um valor total debitado maior que zero e um valor pago entre zero e o total.');
    const sheet=await ensureManualSheet(sellerId); const existing=sellerPaymentSheetLines().find(x=>x.sheetId===sheet.id && Number(x.rowNumber)===rowNumber); const now=new Date().toISOString(); const line={id:existing?.id||uid(),sheetId:sheet.id,sellerId,rowNumber,productName,brand:'',totalValueBRL:Number(total.toFixed(2)),paidValueBRL:Number(paid.toFixed(2)),balanceBRL:Number(Math.max(total-paid,0).toFixed(2)),status:paid>=total?'CLOSED':'OPEN',notes,updatedBy:currentUser.id,createdAt:existing?.createdAt||now,updatedAt:now};
    const payload={id:line.id,sheet_id:line.sheetId,seller_id:line.sellerId,row_number:line.rowNumber,product_name:line.productName,brand:'',total_value_brl:line.totalValueBRL,paid_value_brl:line.paidValueBRL,balance_brl:line.balanceBRL,status:line.status,notes:line.notes,updated_by:line.updatedBy,created_at:line.createdAt,updated_at:now}; const {error}=await supabaseClient.from('seller_payment_sheet_lines').upsert(payload); if(error)return alert(`Não foi possível salvar a linha: ${error.message}`);
    const rows=sellerPaymentSheetLines(); const idx=rows.findIndex(x=>x.id===line.id); if(idx>=0)rows[idx]=line;else rows.push(line); write('nl_seller_payment_sheet_lines',rows); const filled=rows.filter(x=>x.sheetId===sheet.id&&x.productName&&Number(x.totalValueBRL)>0); await supabaseClient.from('seller_payment_sheets').update({total_value_brl:filled.reduce((n,x)=>n+x.totalValueBRL,0),paid_value_brl:filled.reduce((n,x)=>n+x.paidValueBRL,0),balance_brl:filled.reduce((n,x)=>n+x.balanceBRL,0),status:'OPEN',closed_at:null,updated_at:now}).eq('id',sheet.id); showToast(line.status==='CLOSED'?'Linha quitada e salva.':'Linha salva no Supabase.'); renderSellerSheetsPage();
}

function addManualSheetRow(sellerId) {
    if(!canEditWgIk()) return showToast('Apenas WG e IK podem editar esta planilha.');
    window.manualSheetExtraRows=window.manualSheetExtraRows||{}; window.manualSheetExtraRows[sellerId]=(window.manualSheetExtraRows[sellerId]||0)+1; renderSellerSheetsPage();
}

function renderSellerSheetsPage() {
    if(!canEditWgIk()) return showToast('Acesso restrito às contas WG e IK.'); const sellers=allSellers(); const sheets=sellerPaymentSheets(); const lines=sellerPaymentSheetLines(); window.manualSheetExtraRows=window.manualSheetExtraRows||{};
    appFrame('Planilhas manuais — WG e IK','Escreva diretamente no card. Cada vendedor começa com uma linha e você pode adicionar quantas linhas precisar.',`<div class="flex flex-wrap gap-2 mb-5"><button id="backWgIkStockBtn" class="small-btn">Voltar ao Estoque WG ↔ IK</button></div><div class="grid grid-cols-1 xl:grid-cols-2 gap-5">${sellers.map(seller=>{const sheet=activeSellerSheetFor(seller.id);const saved=lines.filter(x=>x.sheetId===sheet?.id).sort((a,b)=>Number(a.rowNumber)-Number(b.rowNumber));const extra=window.manualSheetExtraRows[seller.id]||0;const rowCount=Math.max(1,saved.length+extra+1);const rows=Array.from({length:rowCount},(_,i)=>saved.find(x=>Number(x.rowNumber)===i+1)||{rowNumber:i+1,productName:'',totalValueBRL:0,paidValueBRL:0,balanceBRL:0,status:'OPEN',notes:''});const total=saved.reduce((n,x)=>n+Number(x.totalValueBRL||0),0),paid=saved.reduce((n,x)=>n+Number(x.paidValueBRL||0),0),balance=Math.max(total-paid,0);return `<section class="panel glass-panel p-4" data-seller-card="${seller.id}"><div class="flex justify-between items-center mb-3"><div><h2>${esc(seller.name)}</h2><p class="text-xs text-slate-500">Planilha compartilhada · WG + IK</p></div><div class="text-right text-xs"><b>Total debitado: ${money(total)}</b><span class="block text-emerald-700">Total pago: ${money(paid)}</span><span class="block text-amber-700">Saldo: ${money(balance)}</span></div></div><div class="overflow-x-auto"><table class="w-full text-xs"><thead><tr><th class="text-left">#</th><th class="text-left">Produto</th><th class="text-left">Valor total debitado (R$)</th><th class="text-left">Pago acumulado (R$)</th><th class="text-left">Saldo</th><th class="text-left">Observação</th><th></th></tr></thead><tbody>${rows.map(x=>{const disabled=x.status==='CLOSED'?'disabled':'';return `<tr class="border-b border-slate-100"><td class="font-bold text-slate-400 pr-2">${x.rowNumber}</td><td class="pr-2"><input ${disabled} class="control text-xs min-w-[130px]" data-sheet-field="product-${seller.id}-${x.rowNumber}" value="${esc(x.productName)}" placeholder="Nome do produto"></td><td class="pr-2"><input ${disabled} class="control text-xs w-32" data-sheet-field="total-${seller.id}-${x.rowNumber}" type="number" min="0" step="0.01" value="${Number(x.totalValueBRL||0)||''}" placeholder="0,00"></td><td class="pr-2"><input ${disabled} class="control text-xs w-32" data-sheet-field="paid-${seller.id}-${x.rowNumber}" type="number" min="0" step="0.01" value="${Number(x.paidValueBRL||0)||''}" placeholder="0,00"></td><td class="font-bold pr-2 ${x.balanceBRL<=0&&x.totalValueBRL>0?'text-emerald-700':'text-amber-700'}">${x.totalValueBRL?money(x.balanceBRL):'—'}</td><td class="pr-2"><input ${disabled} class="control text-xs min-w-[120px]" data-sheet-field="notes-${seller.id}-${x.rowNumber}" value="${esc(x.notes||'')}" placeholder="Observação"></td><td>${x.status==='CLOSED'?'<span class="text-emerald-700 font-bold">OK</span>':`<button class="small-btn save-sheet-line" data-seller="${seller.id}" data-row="${x.rowNumber}">Salvar</button>`}</td></tr>`;}).join('')}</tbody></table></div><button class="small-btn add-sheet-row mt-3" data-seller="${seller.id}">+ Adicionar mais uma linha</button></section>`;}).join('')}</div>`);
    document.getElementById('backWgIkStockBtn')?.addEventListener('click',()=>{activeTab='wgIkStock';renderWgIkStockPage();}); document.querySelectorAll('.save-sheet-line').forEach(btn=>btn.addEventListener('click',()=>saveManualSheetLine(btn.dataset.seller,Number(btn.dataset.row),btn.closest('[data-seller-card]')))); document.querySelectorAll('.add-sheet-row').forEach(btn=>btn.addEventListener('click',()=>addManualSheetRow(btn.dataset.seller)));
}
