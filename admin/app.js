...
async function openIkSellerDebitModal(sellerId) {
    if(!isIKAccount()&&!hasAdminAccess()) return showToast('Somente IK pode registrar abatimentos.');
    const seller=allSellers().find(x=>x.id===sellerId); const allocations=ikSellerAllocations().filter(a=>a.sellerId===sellerId&&a.status!=='CLOSED'&&allocationBalance(a)>0).sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt));
    const totalDue=allocations.reduce((n,a)=>n+allocationBalance(a),0); const rate=Number(currentExchangeRate.ask||currentExchangeRate.bid||0);
    if(!seller||!(totalDue>0)) return showToast('Este vendedor não possui valor em aberto.');
    const m=modal(`<h2>Abater valor — ${esc(seller.name)}</h2><p class="text-xs text-slate-500 mb-3">Digite o valor total abatido. O sistema distribuirá o pagamento automaticamente entre os produtos mais antigos, sem exigir valor por produto.</p><div class="p-3 rounded-xl bg-slate-50 border border-slate-200 text-sm mb-3"><div class="flex justify-between"><span>Total devido</span><b>${money(totalDue)} · US$ ${(rate?totalDue/rate:0).toFixed(2)}</b></div></div><form id="ikSellerDebitForm" class="seller-form"><label>Moeda<select name="currency" class="control"><option value="BRL">Real (R$)</option><option value="USD">Dólar (US$)</option></select></label><label>Valor abatido<input name="amount" type="number" min="0.01" step="0.01" class="control" required placeholder="Digite o valor total abatido"></label><label>Observação<input name="notes" class="control"></label><button class="primary-btn w-full mt-3" type="submit">Abater valor do total</button></form>`);
    m.querySelector('form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const currency=String(f.get('currency')||'BRL');const entered=Number(f.get('amount'));const amountBRL=currency==='USD'?entered*rate:entered;if(!(entered>0)||!(amountBRL>0)||amountBRL>totalDue+0.001)return alert(`Valor inválido. Total em aberto: ${money(totalDue)}.`);let remaining=Number(amountBRL.toFixed(2));const now=new Date().toISOString();const written=[];
        for(const a of allocations){if(remaining<=0)break;const part=Number(Math.min(remaining,allocationBalance(a)).toFixed(2));if(!(part>0))continue;const paid=Number((allocationPaidValue(a.id)+part).toFixed(2));const newBalance=Number(Math.max(Number(a.totalValueBRL)-paid,0).toFixed(2));const closed=newBalance<=0;const p={id:uid(),allocationId:a.id,sellerId,ikId:ikAccountId(),amountBRL:part,paymentDate:new Date().toISOString().slice(0,10),frequency:'OTHER',notes:`Abatimento manual ${currency}: ${entered.toFixed(2)}${f.get('notes')?' — '+String(f.get('notes')):''}`,createdBy:currentUser.id,createdAt:now};const pr=await supabaseClient.from('seller_payment_ledger').insert({id:p.id,allocation_id:a.id,sheet_id:null,seller_id:p.sellerId,ik_id:p.ikId,amount_brl:p.amountBRL,payment_currency:currency,original_amount:Number(entered.toFixed(2)),exchange_rate_brl:Number(rate.toFixed(6)),payment_date:p.paymentDate,frequency:p.frequency,notes:p.notes,created_by:p.createdBy,created_at:p.createdAt});if(pr.error)return alert(`Pagamento não salvo: ${pr.error.message}`);const ar=await supabaseClient.from('ik_seller_allocations').update({remaining_value_brl:newBalance,status:closed?'CLOSED':'PARTIAL',updated_at:now}).eq('id',a.id).eq('status',a.status);if(ar.error)return alert(`Saldo não atualizado: ${ar.error.message}`);a.remainingValueBRL=newBalance;a.status=closed?'CLOSED':'PARTIAL';written.push(p);remaining=Number((remaining-part).toFixed(2));}
        const local=sellerPaymentLedger();local.push(...written);write('nl_seller_payment_ledger',local);const localA=ikSellerAllocations();written.forEach(p=>{const a=localA.find(x=>x.id===p.allocationId);if(a){a.remainingValueBRL=Math.max(Number(a.totalValueBRL)-allocationPaidValue(a.id),0);if(a.remainingValueBRL<=0)a.status='CLOSED';}});write('nl_ik_seller_allocations',localA);m.remove();showToast(`Abatimento de ${currency==='USD'?'US$':'R$'} ${entered.toFixed(2)} registrado.`);renderProductsPage();
    };
}
                    ...
                    <div class="seller-card glass-panel p-5 rounded-2xl flex flex-col justify-between gap-3 bg-white/90 border border-slate-200 shadow-sm">
                        <div>
                            ...
                            <div class="p-3 bg-indigo-50/70 rounded-xl mb-3 border border-indigo-100 text-xs">
                                ...
                                <div class="flex justify-between gap-2 mt-1"><span class="text-slate-500 font-semibold">Total em aberto</span><b class="text-amber-700">${moneyPair(sOpenValue)}</b></div>${isIKAccount(currentUser)&&sOpenValue>0?`<button class="small-btn ik-seller-debit-btn mt-3 w-full" data-seller-id="${s.id}">Abater valor (R$ / US$)</button>`:''}
                            </div>
